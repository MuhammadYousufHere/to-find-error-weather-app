...
function updateWeather() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            const minTempEl = document.querySelector(".min");
            const maxTempEl = document.querySelector(".max");
            const weatherIconEl = document.querySelector("i.wi");
            let weatherIcon;
            for (let i = 0; i < weatherIcons.length; i++) {
                for (let j = 0; j < weatherIcons[i].codes.length; j++) {
                    if (data.weather[0].id === weatherIcons[i].codes[j]) {
            ...
            if (data.dt > data.sys.sunset || data.dt < data.sys.sunrise) {
                ...
                weatherIconEl.classList.add(weatherIcon.iconNight);
                lastClass = weatherIcon.iconNight;
                isDay = false;
            } else {
                bodyEl.classList.remove("dark");
                bodyEl.classList.add("light");
                isDay = true;
                weatherIconEl.classList.add(weatherIcon.iconDay);
                lastClass = weatherIcon.iconDay;
            }
